...
UABOUNDARY, CITYBOUNDARY = geographical_boundaries(UA_DIR)
CITYBOUNDARY = CITYBOUNDARY.to_crs(full_im_df_ua.crs)
UABOUNDARY = UABOUNDARY.to_crs(full_im_df_ua.crs)
idINSPIRE_geofull = gpd.sjoin(full_im_df_ua,CITYBOUNDARY,how="left").drop("index_right",axis=1)
...
losses = { "ses_output": "categorical_crossentropy","ua_output": "categorical_crossentropy"}
loss_weights = {"ses_output": 1.0/np.log(NB_SES_CLASSES), "ua_output": 1.0/np.log(NB_UA_CLASS)}
model.compile(optimizer=Adam(lr=INITIAL_LR), loss=losses, loss_weights=loss_weights, metrics=[fmeasure,recall,precision])
...
while global_epoch < MAX_EPOCH:
    ...
    history = model.fit_generator(
        ...
        steps_per_epoch = len(train_generator), #train_image_count // BATCH_SIZE,
        epochs=MAX_EPOCH - global_epoch,
        validation_data=val_generator,
        validation_steps = len(val_generator), #val_image_count // BATCH_SIZE,
        workers=10,
        verbose=1,
...
ses_predictions, ua_predictions = model.predict_generator(test_generator,
                                                          test_image_count, workers=10, verbose=1)
y_true = test_generator.classes
y_true_ses = [ses for ses,ua in y_true]
y_true_ua = [ua - NB_SES_CLASSES  for ses,ua in y_true]
y_pred_ses = np.argmax(ses_predictions, axis=1)
y_pred_ua = np.argmax(ua_predictions, axis=1)
...
print("UA")
print(classification_report(y_true_ua, y_pred_ses))
ua_report = classification_report(y_true_ua, y_pred_ua, output_dict=True)
...
with open(MODEL_OUTPUT_DIR + 'ua_classification_report.csv', 'w') as f:
    for key in ua_report.keys():
        f.write("%s,%s\n" % (key, ua_report[key]))
ua_conf_arr = confusion_matrix(y_true_ua, y_pred_ua)
print(ua_conf_arr)
np.savetxt(MODEL_OUTPUT_DIR + "ua_confusion_matrix.csv", ua_conf_arr, delimiter=",")
