...
MODEL_OUTPUT_DIR = BASE_DIR + "SATELSES/equirect_proj_test/cnes/data_files/outputs/model_data/efficientnet_keras/1km_2019_income_norm_v2/"
BASE_AERDIR = BASE_DIR + "SATELSES/"
AERIAL_DIR= BASE_AERDIR + "equirect_proj_test/cnes/data_files/outputs/1km_AERIAL_esa_URBAN_ATLAS_FR/"

# Global variables
NB_SES_CLASSES = 5
PATIENCE_BEFORE_STOPPING = 15
PATIENCE_BEFORE_LOWERING_LR = 2 #1
TRAIN_TEST_FRAC = .85
VAL_SPLIT = .2
...
full_im_df_ua = generate_full_idINSPIRE_1km(BASE_DIR, AERIAL_DIR, NB_SES_CLASSES)
...
for fold_id,(train_index, test_index) in enumerate(skf.split(full_im_df_ua.path2im, full_im_df_ua.treated_citywise_income)):
    ...
    tensorboard = TensorBoard(log_dir=MODEL_OUTPUT_DIR+"logs/fold_{}-{}".format(fold_id,time()),
                              ...
                              update_freq = 10)
    reduce_lr = ReduceLROnPlateau('loss', factor=0.25, patience=PATIENCE_BEFORE_LOWERING_LR, min_lr=1e-8)
    ...
    while global_epoch < MAX_EPOCH:
        history = model.fit_generator(
            ...
            steps_per_epoch = 250,#2000#len(train_generator), #train_image_count // BATCH_SIZE,
