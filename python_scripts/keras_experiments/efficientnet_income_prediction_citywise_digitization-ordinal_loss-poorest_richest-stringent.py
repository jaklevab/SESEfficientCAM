...
MODEL_OUTPUT_DIR = BASE_DIR + "SATELSES/equirect_proj_test/cnes/data_files/outputs/model_data/efficientnet_keras/2019_income_norm_v3/"
BASE_AERDIR = BASE_DIR + "SATELSES/"
AERIAL_DIR= BASE_AERDIR + "equirect_proj_test/cnes/data_files/outputs/AERIAL_esa_URBAN_ATLAS_FR/"

# Global variables
NB_SES_CLASSES = 5
PATIENCE_BEFORE_STOPPING = 15 #16 #8
PATIENCE_BEFORE_LOWERING_LR = 4 #1
TRAIN_TEST_FRAC = .8
VAL_SPLIT = .25
...
for fold_id,(train_index, test_index) in enumerate(skf.split(full_im_df_ua.path2im, full_im_df_ua.treated_citywise_income)):
    ...
    train_generator = train_datagen.flow_from_dataframe(
            ...
            color_mode ="rgb",
            shuffle=True,
            classes = [str(k) for k in range(NB_SES_CLASSES)],
    ...
    tensorboard = TensorBoard(log_dir=MODEL_OUTPUT_DIR+"logs/fold_{}-{}".format(fold_id,time()),
                              ...
                              update_freq = 50)
    reduce_lr = ReduceLROnPlateau('val_loss',#'loss',
                                  factor=0.25, patience=PATIENCE_BEFORE_LOWERING_LR, min_lr=1e-8)
    ...
    while global_epoch < MAX_EPOCH:
        history = model.fit_generator(
            ...
            workers=10,
            verbose=1,
            class_weight = {0:2,1:1,2:1,3:1,4:2},
