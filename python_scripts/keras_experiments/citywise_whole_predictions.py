...
OUTPUT_DIR = BASE_DIR + "cnes/data_files/outputs/AERIAL_esa_URBAN_ATLAS_FR/"
MODEL_OUTPUT_DIR = BASE_DIR + "cnes/data_files/outputs/model_data/efficientnet_keras/"
RES_DIR = MODEL_OUTPUT_DIR + "2019_income_norm_v2/"
...
idINSPIRE_full = pd.DataFrame(reduce(lambda left,right: pd.merge(left,right,on=['idINSPIRE']),
...
cities = {
    ...
    if "{}_income_pred.pdf".format(city) not in os.listdir("/warehouse/COMPLEXNET/jlevyabi/tmp/")
...
for city, best_model in best_model_city.items():
    if "full_whole_predicted_values.csv" in os.listdir("{}{}/preds/".format(RES_DIR,city)):
    ...
    current_city_assoc[["idINSPIRE",]+["pred_val_{}".format(k) for k in range(NB_SES_CLASSES)]].to_csv("{}{}/preds/full_whole_predicted_values.csv".format(RES_DIR,city),index=False)
