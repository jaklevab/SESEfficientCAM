...
from aerial_training_utils import generate_full_idINSPIRE_1km, geographical_boundaries, my_preprocessor, fmeasure,recall,precision, fbeta_score
...
OUTPUT_DIR = BASE_DIR + "SATELSES/equirect_proj_test/cnes/data_files/outputs/1km_AERIAL_esa_URBAN_ATLAS_FR/"
...
class GuidedBackprop(SaliencyMask):
    ...
    def __init__(self, model, output_index=0, custom_loss=None):
        #
        model_save_loc = '/tmp/1km_gbbis_keras_cpu_{}_{}.h5'.format(city,output_index)
        session_save_loc = '/tmp/1km_guided_backpropbis_ckpt_cpu_{}_{}'.format(city,output_index)
        graph_save_loc = '/tmp/1km_guided_backpropbis_ckpt_cpu_{}_{}.meta'.format(city,output_index)
        #
...
def individual_rastering(gradcam_bg,t,test_cores,class_,time_id):
    raster_loc = BASE_DIR + 'tmp/1km_new_testbis_cpu_{}_{}_{}.tif'.format(city,time_id,class_)
    poly_loc = BASE_DIR + 'tmp/1km_new_poly2bis_cpu_{}_{}.shp'.format(city,time_id)
...
def individual_totvar(gradcam_gdf,class_poor,grad_cam_poor,class_rich,grad_cam_rich,time_id):
    raster_loc_poor = BASE_DIR + 'tmp/1km_new_testbis_cpu_{}_{}_{}.tif'.format(city,time_id,class_poor)
    raster_loc_rich = BASE_DIR + 'tmp/1km_new_testbis_cpu_{}_{}_{}.tif'.format(city,time_id,class_rich)
    poly_loc = BASE_DIR + 'tmp/1km_new_poly2bis_cpu_{}_{}.shp'.format(city,time_id)
...
def individual_statistics(gradcam_cam_poor,gradcam_bg_poor,class_poor,
    ...
    return (t,test_core,gradcam_gdf,val_Id_carr1km)
    ...
    for id_batch_idx,batch_idx in enumerate(BATCH_IDENTITIES):
        ...
        pre_data = Parallel(n_jobs=n_jobs)(delayed(individual_statistics)
                                       (gradcam_cams_poor[j],gradcam_bgs_poor[j],class_poor,
                                        ...
                                        gdf_full_im_df_sampled.iloc[ind:(ind+1)].Id_carr1km.values[0])
...
if __name__ == '__main__':
    ...
    full_im_df_ua = generate_full_idINSPIRE_1km(BASE_DIR, OUTPUT_DIR, NB_SES_CLASSES)
    city_assoc = pd.read_csv(OUTPUT_DIR + "city_assoc.csv")
    full_im_df_ua = pd.merge(full_im_df_ua,city_assoc,on="Id_carr1km");
    ...
    pickle.dump(test,open(MODEL_OUTPUT_DIR+city+"/preds/1km_urbanization_patterns_cpu_{}_income.p".format(city), "wb"))
