...
new_RES_DIR = BASE_DIR + "SATELSES/equirect_proj_test/cnes/"\
                        +"data_files/outputs/model_data/efficientnet_keras/1km_2019_income_norm_v2/"
NB_SES_VALUES = 5
...
idINSPIRE2GEOM = GeoDataFrame.from_file(BASE_DIR + 'INSEE/2019/1km/shps/Filosofi2015_carreaux_1000m_metropole.shp')
...
full_im_df_ua = gpd.GeoDataFrame(pd.merge(city_assoc,idINSPIRE2GEOM,on="Id_carr1km"),crs=idINSPIRE2GEOM.crs);
...
full_mat = gpd.GeoDataFrame(pd.merge(full_im_df_ua[["Id_carr1km","FUA_NAME","treated_citywise_income","geometry"]],
                                     predicted_values,on="Id_carr1km"),crs=full_im_df_ua.crs)
...
with plt.style.context('dark_background'):
    for city in predicted_cities:
        ...
        f.savefig("/warehouse/COMPLEXNET/jlevyabi/tmp/1km_"+city+"_income_pred_ord_loss.pdf",bbox_inches='tight', pad_inches=0.1);
