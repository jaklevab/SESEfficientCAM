...
full_im_df_ua["log_pop_cnt"] = np.log10(full_im_df_ua.ind_c)
pop_dens = full_im_df_ua["log_pop_cnt"]
dens_class_thresholds = [np.percentile(pop_dens,k) for k in np.linspace(0,100,NB_SES_CLASSES +1 )]
...
test_image_count = 10000 #test_im_df.shape[0]
...
test_generator = test_datagen.flow_from_dataframe(
        dataframe=test_im_df.sample(test_image_count),
        directory=AERIAL_DIR,
        x_col="path2im",
        y_col="treated_density",
        target_size=IMG_SIZE,
