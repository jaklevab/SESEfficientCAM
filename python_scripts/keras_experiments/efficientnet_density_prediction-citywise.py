...
MODEL_OUTPUT_DIR = BASE_DIR + "SATELSES/equirect_proj_test/cnes/data_files/outputs/model_data/efficientnet_keras/density/"
BASE_AERDIR = BASE_DIR + "SATELSES/"
AERIAL_DIR= BASE_AERDIR + "equirect_proj_test/cnes/data_files/outputs/AERIAL_esa_URBAN_ATLAS_FR/"

# Global variables
NB_SES_CLASSES = 5
PATIENCE_BEFORE_STOPPING = 20 #8
PATIENCE_BEFORE_LOWERING_LR = 1 #1
TRAIN_TEST_FRAC = .8
VAL_SPLIT = .25
...
ses_city_intervals = full_im_df_ua.groupby("FUA_NAME")[["ind_c"]].agg(
...
for city in list(ses_city_intervals.index):
    ...
    city_df_new.dropna(subset=["ind_c"],inplace=True)
    density = city_df_new.ind_c
    class_thresholds = ses_city_intervals.ix[city]["ind_c"].values
    x_to_class = np.digitize(density,class_thresholds)
    x_to_class[x_to_class==np.max(x_to_class)] = NB_SES_CLASSES
    city_df_new["treated_citywise_density"] = [ str(y-1) for y in x_to_class ] 
...
train_generator = train_datagen.flow_from_dataframe(
        ...
        x_col="path2im",
        y_col="treated_citywise_density",
...
while global_epoch < MAX_EPOCH:
    ...
    history = model.fit_generator(
        ...
        steps_per_epoch = 100,#2000#len(train_generator), #train_image_count // BATCH_SIZE,
