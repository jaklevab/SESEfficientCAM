...
INSEE_AREA = 200*200
MAX_BS = 15 #((max run with 17) grunch) 35 (rrunch) 
IDEAL_WORKLOAD = 6000
conv_name = 'conv2d_65'
input_name = 'input_1'
...
parser.add_argument('-gpu_id','--gpu_id',help = 'ID of GPU')
...
class GuidedBackprop(SaliencyMask):
    ...
    def __init__(self, model, output_index=0, custom_loss=None):
        ...
        """   
        model.save('/tmp/gbbis_keras.h5') 
        with tf.Graph().as_default(): 
            with tf.Session(config=cfg).as_default(): 
                K.set_learning_phase(0)
                load_model('/tmp/gbbis_keras.h5', custom_objects={"custom_loss":custom_loss,"precision":precision,
                                                                 "recall":recall,
                ...
                saver.save(session, '/tmp/guided_backpropbis_ckpt')
        ...
        with self.guided_graph.as_default():
            ...
            with self.guided_graph.gradient_override_map({'Relu': 'GuidedRelu'}):
                saver = tf.train.import_meta_graph('/tmp/guided_backpropbis_ckpt.meta')
                saver.restore(self.guided_sess, '/tmp/guided_backpropbis_ckpt')
...
def individual_rastering(gradcam_bg,t,test_cores,class_):
    class_dataset = rasterio.open(BASE_DIR + 'tmp/new_testbis_%s.tif'%str(class_),'w',driver='GTiff',
    ...
    test_cores[["ITEM2012","geometry"]].to_file(BASE_DIR + 'tmp/new_poly2bis.shp')
    stats_class = zonal_stats(BASE_DIR + 'tmp/new_poly2bis.shp',BASE_DIR + 'tmp/new_testbis_%s.tif'%str(class_),
...
def individual_totvar(gradcam_gdf,grad_cam_poor,grad_cam_rich):
    if grad_cam_poor.sum() == 0 or grad_cam_rich.sum() == 0:
        ...
        stats_totvar_poor = zonal_stats(
            BASE_DIR + 'tmp/new_poly2bis.shp',BASE_DIR + 'tmp/new_testbis_%s.tif'%str(class_poor),
            stats='sum',raster_out=True,)
        stats_totvar_rich = zonal_stats(
            BASE_DIR + 'tmp/new_poly2bis.shp',BASE_DIR + 'tmp/new_testbis_%s.tif'%str(class_rich),
            stats='sum',raster_out=True,)
...
def serialize_gradcaming(model,sample_census_cell_imnames,indices,ind_batched_list,class_):
    # CAM
    ...
    for i in tqdmn(range(len(ind_batched_list)-1)):
        ...
        masks = [guided_bprop.get_mask(img) for img in tqdmn(batch_sample_census_cell_imgs)]
...
def serialize_treating(model,ua_data,gdf_full_im_df_sampled,indices,ind_list,class_poor,class_rich):
    ...
    gradcam_cams_poor, gradcam_bgs_poor = serialize_gradcaming(model,sample_census_cell_imnames,indices,ind_batched_list,class_poor)
    print("GradCaming the dense")
    gradcam_cams_rich, gradcam_bgs_rich = serialize_gradcaming(model,sample_census_cell_imnames,indices,ind_batched_list,class_rich)
    data = []
    ...
    for ind in tqdmn(ind_list):
        ...
        data.append((ts[ind],test_cores[ind],gradcam_gdf,val_idINSPIRE))
    return data

def serialize_batch_treating(model,ua_data,gdf_full_im_df_sampled,indices,ind_list,class_poor,class_rich,ideal_workload):
    print("Overlaying")
    test_cores = [gpd.overlay(ua_data.iloc[indices[ind]],
                             gdf_full_im_df_sampled.iloc[ind:(ind+1)], how='intersection')
                 for ind in tqdmn(ind_list)]
    ...
    #
    data = []
    BATCH_IDENTITIES = range(0,len(ind_list),ideal_workload)
    for id_batch_idx,batch_idx in enumerate(BATCH_IDENTITIES):
        #
        print("Treating Batch {}/{}".format(id_batch_idx,len(BATCH_IDENTITIES)))
        batch_ind_list = ind_list[batch_idx:(batch_idx+ideal_workload)]
        batch_sample_census_cell_imnames = sample_census_cell_imnames[batch_idx:(batch_idx+ideal_workload)]
        ...
        #
        print("GradCaming the sparse")
        gradcam_cams_poor, gradcam_bgs_poor = serialize_gradcaming(model,batch_sample_census_cell_imnames,batch_indices,
                                                                   ind_batched_list,class_poor)
        print("GradCaming the dense")
        gradcam_cams_rich, gradcam_bgs_rich = serialize_gradcaming(model,batch_sample_census_cell_imnames,batch_indices,
                                                                   ind_batched_list,class_rich)      
        print("Computing raster statistics")
        for j,ind in tqdmn(enumerate(batch_ind_list)):
            #if len(batch_ind_list)!=ideal_workload:
            gradcam_gdf_poor = individual_rastering(gradcam_bgs_poor[j],ts[ind],test_cores[ind],class_poor)
            gradcam_gdf_rich = individual_rastering(gradcam_bgs_rich[j],ts[ind],test_cores[ind],class_rich)
            gradcam_gdf = pd.concat([gradcam_gdf_poor,
                                     gradcam_gdf_rich[['4_sum','4_min','4_max','4_median','4_mean','4_count']]],axis=1)
            tot_var_data, sym_KL = individual_totvar(gradcam_gdf,gradcam_cams_poor[j],gradcam_cams_rich[j])
            gradcam_gdf["totvar"] = tot_var_data
            gradcam_gdf["sym_KL"] = sym_KL
            gradcam_gdf["area"] = gradcam_gdf.geometry.area
            gradcam_gdf["poor_score"] = gradcam_gdf["0_sum"]/gradcam_gdf["area"]
...
if __name__ == '__main__':
    print("GradCAMING {} with model defined in {}".format(city,args.model_dir))
    ...
    efficientnet_model = load_model(MODEL_OUTPUT_DIR+city+"/"+best_model_city,custom_objects=dic_load_model)
    ...
    test = serialize_batch_treating(efficientnet_model,ua_data,gdf_full_im_df_sampled,indices,
                              range(gdf_full_im_df_sampled.shape[0]),class_poor,class_rich,IDEAL_WORKLOAD)
    pickle.dump(test,open(MODEL_OUTPUT_DIR+city+"/preds/urbanization_patterns_{}_income.p".format(city), "wb"))
