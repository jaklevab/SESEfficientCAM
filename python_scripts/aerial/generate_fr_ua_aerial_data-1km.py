...
def subextract_from_aerial_tile(tile_fnames,insee_cells_to_extract):
    ...
    for it in range(insee_cells_to_extract.shape[0]):
        ...
        io.imsave(out_dir+"/FR_URBANATLAS_1km_%s.png"%Id_carr1km,skimage.img_as_ubyte(reorder_img))
    aerial_data.close()
...
def extract_labelled_aerial_imagery(df_fullmerge2insee):
    ...
    prepare_input = [(aerial_fname,
                      gpd.GeoDataFrame(pd.DataFrame([Id_carr1km,insee_geom]).transpose().rename(
                          columns={0:'Id_carr1km',1:'geometry'}),crs={"init":"epsg:3035"}))
                     for aerial_fname, Id_carr1km, insee_geom in tqdm(df_fullmerge2insee.values)]
...
def main():
    ...
    geo_df_full_census_data = gpd.read_file(BASE_DIR+"INSEE/2019/1km/shps/Filosofi2015_carreaux_1000m_metropole.shp")
    geo_df_full_census_data = geo_df_full_census_data.to_crs({"init":"epsg:3035"})
    already_extracted_Id_carr1km = pd.DataFrame([g.split("_")[-1].split(".")[0]
                                   for f in os.listdir(OUTPUT_DIR) for g in os.listdir(OUTPUT_DIR + f) if g.endswith(".png")],
                                   columns=["Id_carr1km"])
    print("Skipping {} already extracted images out of {}".format(already_extracted_Id_carr1km.shape[0],geo_df_full_census_data.shape[0]))
    geo_df_full_census_data = pd.merge(geo_df_full_census_data,already_extracted_Id_carr1km,how='outer', indicator=True)
    ...
    #
    extraction_df = df_insee_cars_2_aerial.groupby(["Id_carr1km"])["NOM"].apply(lambda x : list(x)).reset_index()
    extraction_df["aerial_par"] = [";".join(set(x)) for x in extraction_df.NOM]
    extraction_df = pd.merge(extraction_df,geo_df_full_census_data[["Id_carr1km","geometry"]],on="Id_carr1km")
    #
    merge2insee_id = extraction_df.groupby("aerial_par")["Id_carr1km"].apply(lambda x : list(x)).reset_index()
